...
const BonusView = ({ bonus, build }) => {
  ...
  const handlePress = async (tooltipText) => {
    // Отримання даних з DetailsView
    const getDetailsViewData = () => {
      const detailsBuild = build; // або отримати з контексту, якщо ви використовуєте React Context
      return detailsBuild;
    };
    ...
    console.log('tooltipText:', tooltipText); // Логування tooltipText для перевірки
    ...
    if (typeof levelBase === 'string' && typeof level === 'number') {
      ...
      try {
        ...
        // Логування усієї відповіді для перевірки структури
        console.log('Отримані дані:', data);
        
        // Перевірка і отримання значення за шляхом "response/" + tooltipText
        if (data.response && typeof data.response === 'object') {
          // Розділяємо tooltipText на частини
          ...
          for (const key of keys) {
            ...
            if (value === undefined) {
              ...
              // Встановлюємо помилку у контент, якщо ключ не знайдено
